...
public class BiometricService {
    ...
    private final Map<String, BaseDetector> detectorMap = Map.of("haar",
            new CascadeDetector("src/main/resources/weights/haarcascade_frontalface_alt.xml"),
            "lbp",
            new CascadeDetector("src/main/resources/weights/lbpcascade_frontalface_improved.xml"),
    ...
    private final Map<String, BaseRecognizer> recognizerMap = Map.of("hist",
            new HistogramRecognizer(128), "eigen_yolo",
            EigenFaceRecognizer.fromConfig("src/main/resources/weights/yolo_eigenface.config"),
            "neuralnet_yolo",
            new NeuralNetRecognizer("src/main/resources/weights/yolo_NN_recognizer.onnx", 128));

    private final Map<String, Set<String>> allowedDetectorRecognizer;

    private final Map<String, BaseMetric> metricMap = Map.of( // Used to tell how similar two face
                                                              // embedding vectors are
            "euclidian", new EuclideanDistance(), "cosine", new CosineSimilarity());
    ...
    public BiometricService(SessionManager sessionManager, RosterManager rosterManager,
            AttendanceManager attendanceManager, AttendanceRepository attendanceRepository,
        ...
        this.allowedDetectorRecognizer = Map.of("hist", detectorMap.keySet(), // hist works with all
                                                                              // detectors
    ...
    private String resolveDetectorType(String detector_type) {
        ...
        if (!detectorMap.containsKey(detector_type)) {
            throw new IllegalArgumentException(
                    "Invalid detector type. Allowed values: " + detectorMap.keySet());
        }
    ...
    private String resolveRecognizerType(String recognizer_type) {
        ...
        if (!validRecognizers.contains(recognizer_type)) {
            throw new IllegalArgumentException(
                    "Invalid recognizer type. Allowed values: " + validRecognizers);
        }
    ...
    private void validateDetectorRecognizerCompatibility(String detector_type,
            String recognizer_type) {
        ...
        if (allowedDetectors == null || !allowedDetectors.contains(detector_type)) {
            throw new UnsupportedOperationException(String.format(
                    "Recognizer '%s' is not compatible with detector '%s'. Allowed detectors for this recognizer: %s",
                    recognizer_type, detector_type,
                    allowedDetectorRecognizer.get(recognizer_type)));
        }
    }
    ...
    private BaseMetric resolveMetric(String metric_name) {
        ...
        if (metric == null) {
            throw new IllegalArgumentException(
                    "Invalid metric. Allowed values: " + metricMap.keySet().stream().toList());
        }
    ...
    public List<DetectionResultDTO> detect(MultipartFile image, String type) throws IOException {
        ...
        if (detector == null) {
            throw new IllegalArgumentException("Invalid detector type. Allowed values: "
                    + detectorMap.keySet().stream().toList());
        }

        List<DetectionResult> results = detector.detect(imageMat);

        return results.stream().map(DetectionResult::toDTO).toList();
    }

    public List<DetectionResultDTO> detect(byte[] imageBytes, String type) throws IOException {
        // Convert byte[] to MultipartFile using MockMultipartFile
        MultipartFile multipartFile =
                new MockMultipartFile("image", "image.jpg", "image/jpeg", imageBytes);
    ...
    private List<FaceData> getFaceData(Roster roster) {
        ...
        List<FaceData> faceDataList =
                students.stream().flatMap(s -> s.getFaceDataList().stream()).toList();

        if (faceDataList.isEmpty())
            throw new IllegalArgumentException(String.format(
                    "No reference images found for recognition (Roster #%d)", roster.getId()));
    ...
    private Attendance updateAttendance(Session session, Student student, double score,
            Double manualThreshold, Double autoThreshold) {
        ...
        if (score > autoThreshold) {
            ...
            return attendanceManager.updateAttendanceStatusBySessionAndStudent(session.getId(),
                    student.getId(), status, "AUTO", score);
        } else {
            return null;
        }
    ...
    private Optional<Mat> loadBestCrop(FaceData faceData, BaseDetector detector) {
        ...
        return detector.detect(img).stream().max(Comparator.naturalOrder()) // best detection
                .map(det -> ImageUtils.crop(det, img));
    }

    // Build dataset for non-vector recognizers
    private Map<Student, List<Mat>> buildImageDataset(List<FaceData> faceDataList,
            BaseDetector detector) {
        Map<Student, List<Mat>> dataset = new HashMap<>();

        for (FaceData fd : faceDataList) {
            loadBestCrop(fd, detector).ifPresent(crop -> dataset
                    .computeIfAbsent(fd.getStudent(), s -> new ArrayList<>()).add(crop));
        }
        return dataset;
    }

    // Build dataset for vector recognizers (including DB caching)
    private Map<Student, List<double[]>> buildVectorDataset(List<FaceData> faceDataList,
            String detectorType, String recognizerType, BaseDetector detector,
            VectorRecognizer recognizer) {
        Map<Student, List<double[]>> dataset = new HashMap<>();

        // Loop through each facedata and check if a embedding has already been computed for the
        // corresponding facedata
        // If it has, skip the computation and add the saved embedding
        // If not, compute the embedding and save it to the database so it does not have to be
        // recomputed in the future.
        for (FaceData fd : faceDataList) {
            Optional<Embedding> optEmbedding = embeddingRepository
                    .findByDetectorAndRecognizerAndFaceData(detectorType, recognizerType, fd);
            ...
            if (optEmbedding.isPresent()) {
                ...
                embeddingRepository
                        .save(new Embedding(detectorType, recognizerType, fd, embedding));
            }
    ...
    private void addMissingReferenceWarnings(Map<String, String> warnings, List<Student> students,
            Map<Student, ?> dataset) {
        ...
        if (missingStudents.size() == students.size()) {
            ...
            warnings.put("Missing Reference Images",
    ...
    public RecognitionResponse recognize(MultipartFile image, long session_id, String detector_type,
            String type, String metric_name, // Note: Metric has no effect on Histogram recognizers
                                             // as they do not produce image embedding vectors.
            Double manualThreshold, Double autoThreshold) throws IOException {
        ...
        BaseRecognizer recognizer =
                recognizerMap.getOrDefault(type, recognizerMap.get(type + "_" + detector_type));
        if (recognizer == null)
            throw new IllegalArgumentException(
                    "Invalid recognizer type. Allowed values: " + validRecognizers);
        ...
        // Check if recognizer produces a embedding vector which is used for recognition. Eg.
        // EigenFace/NeuralNet
        if (recognizer instanceof VectorRecognizer vectorRecognizer) {
            vectorRecognizer.setMetric(metric);

            Map<Student, List<double[]>> vectors = buildVectorDataset(faceDataList, detector_type,
                    type, detector, vectorRecognizer);
            ...
            for (DetectionResult det : detectionResults) {
                ...
                Attendance att =
                        updateAttendance(session, best, score, manualThreshold, autoThreshold);
                results.add(new RecognitionResultDTO(det.toDTO(), convertToDTO(best), score,
                        convertToDTO(att)));
            }
    ...
    @Transactional
    public RecognitionResponse recognize(byte[] imageBytes, long session_id, String detector_type,
            String type, String metric_name, Double manualThreshold, Double autoThreshold)
            throws IOException {
        // Convert byte[] to MultipartFile using MockMultipartFile
        MultipartFile multipartFile =
                new MockMultipartFile("image", "image.jpg", "image/jpeg", imageBytes);

        return recognize(multipartFile, session_id, detector_type, type, metric_name,
                manualThreshold, autoThreshold);
    }
